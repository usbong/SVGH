...
/*
' Given:
' 1) .txt format of the Portable Document Format (PDF) file
' --> At present, this computer software automation tool accepts as input files from the following Health Maintenance Organization (HMO): 
' --> Maxicare
'
' Output:
' Part I.
' 1) Auto-generated paid HMO Summary Report
' --> "Tab delimited" .txt file 
' --> Regardless of the name of the input file or input files, the output file will be "paidHMOSummaryReportOutputMaxicare.txt".
'
' Notes:
' 1) To execute the add-on software/application simply use the following command:
'   java generateUnpaidHMOSummaryReportOfAllInputFilesSVGH input201801.txt
' 
...
'   java generateUnpaidHMOSummaryReportOfAllInputFilesSVGH input*
' 
' where: "input*" means any file in the directory that starts with "input".
'
' 3) Make sure to include the HMO name, e.g. "Maxicare" in the input file name.
' --> This is so that the add-on software would be able to properly identify it as a set of "Maxicare" transactions.
' --> Example: inputMaxicare201801.txt
'
' 4) If you use space in your file name, e.g. "input Maxicare 201801.txt", you will have to execute the input files as follows.
'   java generateUnpaidHMOSummaryReportOfAllInputFilesFromMasterList *"2018"*.txt
'
' where: * means any set of characters
'
' 5) We presently use this computer software automation tool with our partner, St. Vincent General Hospital (SVGH) in Marikina City.
' --> We should be able to reuse this tool for the rest of our partners that need technology assistance in automatically processing Health Maintenance Organization (HMO) payments.
*/ 

//TO-DO: -update: the instructions
public class generateUnpaidHMOSummaryReportOfAllInputFilesSVGH {	
